...
::: {.screen-only}
This page serves the whole book in one document, so that a single call of 'print' 
gives you everything rather than one chapter at a time. Nothing here is a second copy:
the chapters are pulled straight from the same sources as the chapter pages,
so this page cannot fall behind them. There's a few hacks additional hacks I applied
(deleted the search-bar icon, moved the footnotes to not all be cluttered at the end, 
small changes like this). Just make sure to update the chapter list on this page, and the
print should follow along just fine. 
